...
def draw_model(
    ...
    def run(model, repeat: int = 1):
        for i in range(max(len(input_data), repeat)):
...
def default_draw_model(
        ...
        jit: str = "none",
        warmup_steps: int = 1,
        profiled_run_number: int = 1,
) -> None:
    profiled_kernels_groups = []
    fw_profiled_kernels = [
        k for k in profiled_kernels if f"fw-graph{profiled_run_number}" in k._nvtx
    ]
    bw_profiled_kernels = [
        k for k in profiled_kernels if f"bw-graph{profiled_run_number}" in k._nvtx
    ]
    ...
    if jit == "aot":
        ...
        if len(fw_profiled_kernels) > 0:
            profiled_kernels_groups.append(fw_profiled_kernels)
        if len(bw_profiled_kernels) > 0:
            profiled_kernels_groups.append(bw_profiled_kernels)
